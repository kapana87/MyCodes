...
class HistoryDict:
    def __init__(self, data=None) -> None:
        ...
        self._copy_data = {key: [value] for key, value in deepcopy(self._data).items()}
    ...
    def __setitem__(self, key, value):
        ...
        if key in self._copy_data:
            self._copy_data.setdefault(key, value).append(value)
        self._copy_data.setdefault(key, [value])

    def __delitem__(self, key):
        if key in self._data:
            del self._data[key]
        if key in self._copy_data:
            del self._copy_data[key]
